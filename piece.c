...
bool intersect(cpiece p1, cpiece p2){
	...
	if (is_horizontal(p1))
		if (is_horizontal(p2)) // Si les 2 sont Horizontales, on vérifie si elles se croisent
			if (get_x(p1) != get_x(p2)) // Leurs coordonnées verticales
				return false ;
			if (get_y(p1) == get_y(p2)) // Leurs coordonnées horizontales
				return true ;
			if (get_y(p1) > get_y(p2)) 
				return get_y(p1) < get_y(p2) + size_p2 ; // On vérifie que le point de gauche de p1 est contenu dans p2
		...
		if (!is_horizontal(p2)) // Si les 2 sont Verticales, on vérifie si elles se croisent
			...
			if (get_x(p1) > get_x(p2)) 
				return get_x(p1) < get_x(p2) + size_p2 ; // On vérifie que le point du de p1 est contenu dans p2
